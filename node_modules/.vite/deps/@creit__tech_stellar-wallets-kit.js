...
var require_index_min = __commonJS({
  "node_modules/@stellar/freighter-api/build/index.min.js"(exports, module) {
    ...
    !function(r3, e2) {
      ...
      const n2 = (r4) => {
        ...
        if (w2) {
          const t3 = await (async (r5, s4) => {
            let t4, o2 = "", T2 = "", A2 = "";
            "object" == typeof s4 ? (T2 = s4.accountToSign || "", A2 = s4.networkPassphrase || "") : (o2 = s4 || "", T2 = "");
            try {
              t4 = await n2({ transactionXdr: r5, network: o2, networkPassphrase: A2, accountToSign: T2, type: e2.SUBMIT_TRANSACTION });
            } catch (r6) {
              return { signedTransaction: "", signerAddress: "", error: E };
            }
            const { signedTransaction: _3, signerAddress: i3 } = t4;
            return { signedTransaction: _3, signerAddress: i3, error: null == t4 ? void 0 : t4.apiError };
          })(r4, s3);
        ...
        if (w2) {
          const t3 = await (async (r5, s4, t4) => {
            let o2;
            const T2 = (t4 || {}).address || "";
            try {
              o2 = await n2({ blob: r5, accountToSign: T2, apiVersion: s4, type: e2.SUBMIT_BLOB });
            } catch (r6) {
              return { signedMessage: null, signerAddress: "", error: E };
            }
            const { signedBlob: A2, signerAddress: _3 } = o2;
            return { signedMessage: A2 || null, signerAddress: _3, error: null == o2 ? void 0 : o2.apiError };
          })(r4, "4.0.0", s3);
          return t3.error ? { signedMessage: null, signerAddress: "", error: t3.error } : { signedMessage: t3.signedMessage, signerAddress: t3.signerAddress };
        }
        ...
        if (w2) {
          const t3 = await (async (r5, s4) => {
            ...
            let o2;
            try {
              o2 = await n2({ entryXdr: r5, accountToSign: t4, networkPassphrase: null == s4 ? void 0 : s4.networkPassphrase, type: e2.SUBMIT_AUTH_ENTRY });
            } catch (r6) {
          ...
          return t3.error ? { signedAuthEntry: null, signerAddress: "", error: t3.error } : { signedAuthEntry: t3.signedAuthEntry, signerAddress: t3.signerAddress };
        }
        return { signedAuthEntry: null, signerAddress: "", error: o };
      }, d2 = async () => w2 ? window.freighter ? Promise.resolve({ isConnected: window.freighter }) : (async () => {
      ...
      })() : { isConnected: false, error: o }, l = async () => {
        if (w2) {
        ...
        if (w2) {
          const s3 = await (async () => {
            let r5;
            try {
              r5 = await n2({ type: e2.REQUEST_ALLOWED_STATUS });
            } catch (r6) {
              console.error(r6);
            }
        ...
        if (w2) {
          const s3 = await (async () => {
            let r5;
            try {
              r5 = await n2({ type: e2.REQUEST_ACCESS });
            } catch (r6) {
              console.error(r6);
            }
            const { publicKey: s4 } = r5 || { publicKey: "" };
            return { publicKey: s4, error: null == r5 ? void 0 : r5.apiError };
          })();
          return r4 = s3.publicKey, s3.error ? { address: r4, error: s3.error } : { address: r4 };
        }
        return { address: r4, error: o };
      };
      class O {
        ...
        watch(r4) {
          return w2 ? (this.isRunning = true, this.fetchInfo(r4), {}) : { error: o };
        }
        stop() {
          this.isRunning = false;
        }
      }
      const w2 = "undefined" != typeof window, u = { getAddress: _2, signTransaction: i2, signMessage: a, signAuthEntry: S, isConnected: d2, getNetwork: l, getNetworkDetails: N, isAllowed: c, setAllowed: C, requestAccess: R, WatchWalletChanges: O };
      return t2;
    })());
  }
});
...
var FreighterModule = class {
  ...
  async getAddress() {
    return this.runChecks().then(() => (0, import_freighter_api.requestAccess)()).then(() => (0, import_freighter_api.getAddress)()).then(({ address, error }) => {
  ...
  async signMessage(message, opts) {
    return this.runChecks().then(async () => {
      ...
      return { signedMessage: Buffer$1.from(signedMessage).toString("base64"), signerAddress };
    }).catch((e2) => {
      throw parseError(e2);
    });
  }
...
var L = ((L2) => (L2.XBULL_CONNECT = "XBULL_CONNECT", L2.XBULL_GET_PUBLIC_KEY = "XBULL_GET_PUBLIC_KEY", L2.XBULL_SIGN_XDR = "XBULL_SIGN_XDR", L2.XBULL_GET_NETWORK = "XBULL_GET_NETWORK", L2.XBULL_INITIAL_RESPONSE = "XBULL_INITIAL_RESPONSE", L2.XBULL_CONNECT_RESPONSE = "XBULL_CONNECT_RESPONSE", L2.XBULL_SIGN = "XBULL_SIGN", L2.XBULL_SIGN_RESPONSE = "XBULL_SIGN_RESPONSE", L2))(L || {});
function _(L2) {
  return !!L2.error;
}
...
var r = class {
  ...
  sendEventToContentScript(e2, r3, o) {
    return new Promise((t2) => {
      const n2 = (e3) => {
        ...
        e3.data.eventId === o && (t2(e3), window.removeEventListener("message", n2, false));
      };
      window.addEventListener("message", n2, false), window.webkit.messageHandlers.cordova_iab.postMessage(JSON.stringify({ type: e2, eventId: o, detail: r3 }));
    });
  }
  async enableConnection() {
    const r3 = { origin: window.origin, host: window.location.host, permissions: { canRequestPublicKey: true, canRequestSign: true } }, o = await this.sendEventToContentScript(L.XBULL_CONNECT, r3, crypto.randomUUID()), { detail: t2 } = o.data;
    if (!t2 || t2.error) throw { code: t2?.code || -1, message: t2?.errorMessage || "Unexpected error" };
  ...
  async getAddress() {
    ...
    const r3 = { origin: window.origin, host: window.location.host }, o = await this.sendEventToContentScript(L.XBULL_GET_PUBLIC_KEY, r3, crypto.randomUUID()), { detail: t2 } = o.data;
    return !t2 || t2.error ? { error: { code: t2?.code || -1, message: t2?.errorMessage || "Unexpected error" } } : { address: t2.payload };
  }
  async signTransaction(r3) {
    ...
    const o = { origin: window.origin, host: window.location.host, network: r3.opts?.networkPassphrase, publicKey: r3.opts?.address, xdr: r3.xdr, xdrType: "Transaction" }, t2 = await this.sendEventToContentScript(L.XBULL_SIGN_XDR, o, crypto.randomUUID()), { detail: n2 } = t2.data;
    return !n2 || n2.error ? { error: { code: n2?.code || -1, message: n2?.errorMessage || "Unexpected error" } } : { signedTxXdr: n2.payload.signedXdr, signerAddress: n2.payload.signerAddress };
  }
  async getNetwork() {
    try {
      await this.enableConnection();
    ...
    const r3 = { origin: window.origin, host: window.location.host }, o = await this.sendEventToContentScript(L.XBULL_GET_NETWORK, r3, crypto.randomUUID()), { detail: t2 } = o.data;
    return !t2 || t2.error ? { error: { code: t2?.code || -1, message: t2?.errorMessage || "Unexpected error" } } : { network: t2.payload.network, networkPassphrase: t2.payload.networkPassphrase };
  }
};
...
var w = class {
  constructor(o) {
    ...
    const u = import_tweetnacl.box.keyPair(), d2 = (0, import_tweetnacl_util.encodeBase64)((0, import_tweetnacl.randomBytes)(24));
    this.encryptForReceiver = (r3) => {
      const n2 = (0, import_tweetnacl.randomBytes)(24), o2 = (0, import_tweetnacl.box)((0, import_tweetnacl_util.decodeUTF8)(r3.data), n2, r3.receiverPublicKey, u.secretKey);
      ...
      const t2 = import_tweetnacl.box.open((0, import_tweetnacl_util.decodeBase64)(s2.payload), (0, import_tweetnacl_util.decodeBase64)(s2.oneTimeCode), (0, import_tweetnacl_util.decodeBase64)(s2.senderPublicKey), u.secretKey);
      if (!t2) throw new Error("Decrypted message is null");
      return (0, import_tweetnacl_util.encodeUTF8)(t2);
    }, this.publicKey = () => u.publicKey, this.session = () => d2.slice();
    const b = (e2) => {
      switch (e2.data.type) {
